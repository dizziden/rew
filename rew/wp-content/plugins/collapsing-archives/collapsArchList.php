...
function list_archives($options) {
  ...
  foreach( $allPosts as $archPost ) {
    ...
    if ($currentYear != $archPost->year ) {
      ...
      if( $expandYears  || $expandMonths) {
        $archives .= "  <li class='acollapsing archives $yearRel'><span title='$yearTitle' " .
            "class='collapsing archives $yearRel'>" .
            "<span class='sym'>$ding</span>";
      } else {
        $archives .= "  <li class='acollapsing archives item'>\n";
      }
      if (in_array($post_type, array('all', 'post'))) {
        $yearLink = "<a href='".get_year_link($archPost->year). "' class=\"yearat\">$currentYear $yearCount</a>\n";
      } else {
  ...
  if ($currentMonth != $archPost->month) {
    ...
    if( $expandYears ) {
      ...
      $archives .= "      <li class='acollapsing archives $monthRel'>".$the_link;
  ...
  $text = '';
    ...
    if ($showPostTitle  && $expandMonths) {
      ...
      $monthText .= "          <li class='acollapsing archives item'><a href='$link' " .
