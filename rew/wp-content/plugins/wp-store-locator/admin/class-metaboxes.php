        ...
        public function meta_box_fields() {
            ...
            $meta_fields = array(
                ...
                __( 'Additional Information', 'wpsl' ) => array(
                    ...
                    'url' => array(
                        'label' => __( 'Url', 'wpsl' )
                    ),					'hour_opration_title' => array(                        'label' => __( 'Hours Opration Title', 'wpsl' )                    ),					'hour_opration_text' => array(                        'label' => __( 'Hours Opration Text', 'wpsl' )                    )					,					'credit_button' => array(                        'label' => __( 'Credi Botton Text', 'wpsl' )                    ),					'credit_button_link' => array(                        'label' => __( 'Credi Botton Link', 'wpsl' ),						'type' => 'media'						                    ),					'application_address' => array(                        'label' => __( 'Application Address', 'wpsl' )                    )
                )
            );
                ...
                foreach ( $this->meta_box_fields() as $tab => $meta_fields ) {
                    ...
                    if ( $wpsl_settings['hide_hours'] && $tab == __( 'Opening Hours', 'wpsl' ) ) {
                        ...
                        foreach ( $meta_fields as $field_key => $field_data ) {
							...
							if($field_data['type'] == 'media'){								?>                                <label for="upload_image">Credi Botton Link</label>                                    <input id="upload_image" type="text" size="36" name="wpsl[credit_button_link]" value="http://" />                                     <input id="upload_image_button" class="button"   type="button" value="Upload Image" />                                                             <?php 																	}
                            ...
                            default:
                                if ( is_array( $this->store_data[ $field_key ] ) ) {
                                    if ( wpsl_is_multi_array( $this->store_data[ $field_key ] ) ) {
                                        ...
                                        update_post_meta( $post_id, 'wpsl_' . $field_key, $this->store_data[ $field_key ] );																				if($field_data['type'] == 'media'){											update_post_meta( $post_id, 'wpsl_credit_button_link', $this->store_data[ 'wpsl_credit_button_link' ] );										}
                                    } else {
