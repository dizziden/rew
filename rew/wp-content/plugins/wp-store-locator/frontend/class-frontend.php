...
if ( !class_exists( 'WPSL_Frontend' ) ) {
    ...
    class WPSL_Frontend {
		...
		public function __construct() {
            ...
            add_shortcode( 'wpsl_onlymap',                     array( $this, 'show_store_locator_onlymap' ) );
            add_shortcode( 'wpsl_onlylist',                     array( $this, 'show_store_locator_list' ) );
        ...
        public function frontend_meta_fields() {
            ...
            $store_fields = array(
				...
				'wpsl_hour_opration_title' => array(
                    'name' => 'hour_opration_title'
                ),
				'wpsl_hour_opration_text' => array(
                    'name' => 'hour_opration_text'
                ),
				'wpsl_credit_button' => array(
                    'name' => 'credit_button'
                ),
				'wpsl_credit_button_link' => array(
                    'name' => 'credit_button_link',
					'type' => 'media'
                ),
				'wpsl_application_address' => array(
		...
		public function show_store_locator( $atts ) {
		  ...
		  /**
         * Handle the [wpsl_onlymap] shortcode.
         * 
         * @since 2.0.0
         * @todo   add schema.org support.
         * @param  array       $atts   Shortcode attributes
		...
		public function show_store_locator_onlymap( $atts ) {
            ...
            // Check if we have a template path and the file exists, otherwise we use the default template.
            if ( !$template_path || ( !file_exists( $template_path ) ) ) {
                $template_path = WPSL_PLUGIN_DIR . 'frontend/templates/onlymap.php';
            }
		  ...
		  /**
         * Handle the [wpsl_onlylist] shortcode.
         * 
         * @since 2.0.0
         * @todo   add schema.org support.
         * @param  array       $atts   Shortcode attributes
         ...
         */
			...
			public function show_store_locator_list( $atts ) {
            ...
            // Check if we have a template path and the file exists, otherwise we use the default template.
            if ( !$template_path || ( !file_exists( $template_path ) ) ) {
                $template_path = WPSL_PLUGIN_DIR . 'frontend/templates/onlylist.php';
            }
        ...
        public function show_store_address( $atts ) {
            ...
            $atts = wpsl_bool_check( shortcode_atts( apply_filters( 'wpsl_address_shortcode_defaults', array(
                ...
                'email'    => true,
                'url'      => true,
				'hour_opration_title' => true,
				'hour_opration_text' => true,
				'credit_button' => true,
				'credit_button_link' => true,
				'application_address' => true
            ) ), $atts ) );
            ...
            if ( $atts['phone'] || $atts['fax'] || $atts['email'] || $atts['url'] ) {
                ...
                if ( $atts['url'] && $store_url = get_post_meta( $atts['id'], 'wpsl_url', true ) ) {
				 ...
				 if ( $atts['hour_opration_title'] && $store_hour_opration_title = get_post_meta( $atts['id'], 'wpsl_hour_opration_title', true ) ) {
                     
                    $content   .= esc_html( $wpsl->i18n->get_translation( 'hour_opration_title_label', __( 'Hours Of Opration', 'wpsl' ) ) ) . ': ' . esc_url( $store_hour_opration_title ) . '<br/>';
                }
				
				 if ( $atts['hour_opration_text'] && $store_hour_opration_text = get_post_meta( $atts['id'], 'wpsl_hour_opration_text', true ) ) {
                    
                    $content   .= esc_html( $wpsl->i18n->get_translation( 'hour_opration_text_label', __( 'Text', 'wpsl' ) ) ) . ': ' . esc_url( $store_hour_opration_text ) . '<br/>';
                }
				 if ( $atts['credit_button'] && $store_credit_button = get_post_meta( $atts['id'], 'wpsl_credit_button', true ) ) {
                     
                    $content   .= esc_html( $wpsl->i18n->get_translation( 'credit_button_label', __( 'Button', 'wpsl' ) ) ) . ': ' . esc_url( $store_credit_button ) . '<br/>';
                }
				
				
			 	   if ( $atts['credit_button_link'] && $store_credit_button_link = get_post_meta( $atts['id'], 'wpsl_credit_button_link', true ) ) {
                    $new_window = ( $wpsl_settings['new_window'] ) ? 'target="_blank"' : '' ;
                    $content   .= esc_html( $wpsl->i18n->get_translation( 'credit_button_link_label', __( 'Link', 'wpsl' ) ) ) . ': <a ' . $new_window . ' href="' . esc_url( $store_credit_button_link ) . '">' . esc_url( $store_credit_button ) . '</a><br/>';
                }
				
				 
				 
				
				 if ( $atts['application_address'] && $store_application_address = get_post_meta( $atts['id'], 'wpsl_application_address', true ) ) {
                     
                    $content   .= esc_html( $wpsl->i18n->get_translation( 'application_address_label', __( 'Application Address', 'wpsl' ) ) ) . ': ' . esc_url( $store_application_address ) . '<br/>';
                }
