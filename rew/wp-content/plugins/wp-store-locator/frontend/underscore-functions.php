...
<script id="wpsl-listing-template" type="text/template">
    <?php
        $listing_template = '<li data-store-id="<%= id %>" class="panel panel-default">' . "\r\n";				$listing_template .= '<a data-toggle="collapse" data-parent="#accordion" href="#collapse<%= id %>">' . "\r\n";		$listing_template .= "\t\t".'<div class="panel-heading"><h4 class="panel-title">' . "\r\n";
		$listing_template .= "\t\t\t\t" . wpsl_store_header_template( 'listing' ) . "\r\n";	
		$listing_template .= "\t\t" .'</div></a>' . "\r\n"; 
		$listing_template .= "\t\t" . '<div id="collapse<%= id %>" class="panel-collapse collapse">' . "\r\n"; 		$listing_template .= "\t\t\t" . '<div class="panel-body">' . "\r\n";
        $listing_template .= "\t\t" . '<div class="wpsl-store-left">' . "\r\n";	
       ...
       /*  if ( !$wpsl_settings['hide_country'] ) {
        ...
        if ( $wpsl_settings['show_contact_details'] ) {
           ...
           /*  $listing_template .= "\t\t\t" . '<span><strong>' . esc_html( $wpsl->i18n->get_translation( 'phone_label', __( 'Phone', 'wpsl' ) ) ) . '</strong>: <%= formatPhoneNumber( phone ) %></span>' . "\r\n"; */
		    ...
		    $listing_template .= "\t\t\t" . '<span><strong>' . esc_html( $wpsl->i18n->get_translation( 'phone_label', __( 'Phone', 'wpsl' ) ) ) . '</strong>: <a href="tel:<%= phone %>"><%= phone %></a></span>' . "\r\n";
            ...
            $listing_template .= "\t\t\t" . '<span><strong>' . esc_html( $wpsl->i18n->get_translation( 'email_label', __( 'Email', 'wpsl' ) ) ) . '</strong>: <a href="mailto:<%= phone %>"><%= email %></a></span>' . "\r\n";
            $listing_template .= "\t\t\t" . '<% } %>' . "\r\n";
			
			
			
			 $listing_template .= "\t\t\t" . '<% if ( hour_opration_title ) { %>' . "\r\n";
            $listing_template .= "\t\t\t" . '<span><strong><%= hour_opration_title %></strong>:<br> <%= hour_opration_text %></span>' . "\r\n";
            $listing_template .= "\t\t\t" . '<% } %>' . "\r\n";
			
			
			 $listing_template .= "\t\t\t" . '<% if ( credit_button_link ) { %>' . "\r\n";
			$listing_template .= "\t\t\t" . '<a href="<%= credit_button_link %>" target="_blank" class="credit_button_link"><%= credit_button %></a>' . "\r\n";
			 $listing_template .= "\t\t\t" . '<% } %>' . "\r\n";
			
			 $listing_template .= "\t\t\t" . '<% if ( application_address ) { %>' . "\r\n";
            $listing_template .= "\t\t\t" . '<span><strong>Application Address</strong>:<br> <%= application_address %></span>' . "\r\n";
			$listing_template .= "\t\t\t" . '<% } %>' . "\r\n";
		...
		if ( !$wpsl_settings['hide_hours'] ) {
                ...
                $listing_template .= "\t\t\t\t" . '<div class="wpsl-store-hours"><strong>' . esc_html( $wpsl->i18n->get_translation( 'hours_label', __( 'Hours', 'wpsl' ) ) ) . '</strong><%= hours %></div>' . "\r\n";
                $listing_template .= "\t\t\t\t" . '<% } %>' . "\r\n";
            }
		...
		/*  $listing_template .= "\t\t\t\t" . '<% if ( excerpt  ) { %>' . "\r\n";
            $listing_template .= "\t\t\t\t" . '<%= excerpt %>' . "\r\n";
            $listing_template .= "\t\t\t\t" . '<% } %>' . "\r\n"; */
        ...
        $listing_template .= "\t\t" . '</div>' . "\r\n";
        $listing_template .= "\t\t" . '</div>' . "\r\n";
        $listing_template .= "\t\t" . '<div class="wpsl-store-right">' . "\r\n";
		$listing_template .= '<iframe src = "https://maps.google.com/maps?q=<%= address %>&ie=UTF8&t=p&z=15&iwloc=B&output=embed" allowfullscreen></iframe>';	
        $listing_template .= "\t\t" . '</div>' . "\r\n";
    ...
    if ( $wpsl_settings['more_info'] ) {
        ...
        if ( $wpsl_settings['more_info_location'] == 'store listings' ) {
          ...
          //  $more_info_template = '<p><a class="wpsl-store-details" href="' . $more_info_url . '">' . esc_html( $wpsl->i18n->get_translation( 'more_label', __( 'More info', 'wpsl' ) ) ) . '</a></p>';
		    $more_info_template = '<a class="wpsl-store-details" href="' . $more_info_url . '">' . esc_html( $wpsl->i18n->get_translation( 'more_label', __( 'More info', 'wpsl' ) ) ) . '</a>';
        }
