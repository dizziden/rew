		...
		if ( have_posts() ) {
			...
			<?php if ( 'off' === $fullwidth || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) ) { ?>
				...
				<?php
					...
					if ( 'on' !== $show_content ) {
						$more = 'on' == $show_more ? sprintf( '<div class="more-link-holder"><a href="%1$s" class="more-link" >%2$s</a></div>' , esc_url( get_permalink() ), esc_html__( 'read more', 'et_builder' ) )  : '';
						echo $more;
					}
